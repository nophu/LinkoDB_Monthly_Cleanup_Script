...
def validate_records(records, rubric, source_filename):
    ...
    for record in records:
        ...
        for field, value in record.items():
            ...
            # store the cleaned value in the record
    ...
    _print_summary(changes, checkable_fields, source_filename)
    return validated, changes


# helper function for checking a single value against the rubric rules for that field
def _check_value(field, value, rubric):
    ...
    for valid in valid_values:
        if valid.lower() == value_lower: return {  "status": "fixed",  "cleaned_value": valid,  "note": f"fixed casing: '{value}' → '{valid}'" }
    ...
    # no match at all
    return {  "status": "flagged", "cleaned_value": value, "note": f"'{value}' not found in rubric valid values — review manually" }


# helper for finding a partial match between a value and the valid values list
...
# helper for getting the facility name from a record
def _get_facility_name(record):
    # try each possible facility name field in order
...
# helper for getting the permit number from a record
def _get_permit_no(record):
    for field in ["txtPermitNo", "PermitNo", "PermitID"]:
        val = record.get(field)
        if val and str(val).strip() not in ("", "nan", "NaN", "None"):  return str(val).strip()
    return "Unknown"


# helper for printing a summary of validation results to the console
def _print_summary(changes, checkable_fields, source_filename):
    ...
    print("=" * 45)
    print(f"\n  Fields checked:     {checkable_fields}")
